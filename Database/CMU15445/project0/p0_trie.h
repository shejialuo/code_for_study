...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (child->GetKeyChar() != key_char || HasChild(key_char)) {
      return nullptr;
    }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *p = ptr;
    for (size_t i = 0; i < key.size() - 1; ++i) {
      p = (*ptr)->GetChildNode(key[i]);
      if (p == nullptr) {
    ...
    if (p == nullptr) {
      ...
      auto end = std::make_unique<TrieNodeWithValue<T>>(std::move(**p), value);
      (*ptr)->RemoveChildNode(key.back());
      (*ptr)->InsertChildNode(key.back(), std::move(end));
    }
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    while (st.top() != &root_) {
      std::unique_ptr<TrieNode> *p = st.top();
      st.pop();
      if (!(*p)->HasChildren() && !(*p)->IsEndNode()) {
        (*(st.top()))->RemoveChildNode((*p)->GetKeyChar());
      }
    }
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    if (ptr == nullptr || !(*ptr)->IsEndNode()) {
      *success = false;
      return T{};
    }

    auto node = dynamic_cast<TrieNodeWithValue<T> *>(&(**ptr));
    if (node == nullptr) {
      *success = false;
      return T{};
    }
